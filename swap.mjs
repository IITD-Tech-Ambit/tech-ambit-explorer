...
if (rightStart > 0 && leftStart > 0 && rightStart > leftStart) {
    ...
    const betweenBlock = content.substring(leftEnd, rightStart);

    let newRightBlock = rightBlock.replace('xl:col-span-1 border-t xl:border-t-0 xl:border-l border-border pt-8 xl:pt-0 xl:pl-8 space-y-6', 'xl:col-span-1 border-b xl:border-b-0 xl:border-r border-border pb-8 xl:pb-0 xl:pr-8 space-y-6');
    let newLeftBlock = leftBlock.replace('xl:col-span-3 space-y-6', 'xl:col-span-3 space-y-6');
    
    // adjust comments implicitly
    ...
    console.log('Could not find boundaries or already swapped');
}
